...
def main():
    ...
    axes[0].set_xlabel("$p_{fav}$ de-vigada"); axes[0].set_ylabel("contribuição FLB ·o")
    axes[0].set_title("Curva mecânica do FLB")
    axes[1].scatter(L.skew_exante, L.flb, s=22, color="#1f77b4", label="FLB total")
    axes[1].scatter(L.skew_exante, L.residual, s=22, color="#d62728", label="resíduo")
    axes[1].axhline(0, color="0.8", lw=1)
    axes[1].set_xlabel("skewness implícita da liga"); axes[1].set_ylabel("retorno")
    axes[1].set_title(f"FLB total (r={rf['r']:+.2f}) vs resíduo (r={rr['r']:+.2f})")
