...
def main():
    ...
    axes[0].set_xlabel("Elo médio do time (dominância)")
    axes[0].set_ylabel("skew média dos jogos do time")
    axes[0].set_title(f"Times: dominância vs skew (r={r['r']:+.2f})")
    axes[1].scatter(M.elo_sd, M.skew_exante, s=22, color="#d62728")
    axes[1].set_xlabel("dispersão de Elo da liga"); axes[1].set_ylabel("skew da liga")
