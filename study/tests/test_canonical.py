"""Testes da camada canônica — sintéticos, SEM dataset (rodam em CI).

Validam o contrato e o núcleo sport-agnóstico em mercados de 3 e de 2 resultados.
...
def _frame(events, outcomes, roles, sport="x", market="m"):
    """events: lista de (odds_dict, winner_outcome). Devolve frame canônico de-vigado."""
    rows = []
...
def test_devig_and_validate_three():
    ...
    # Σp ≈ 1 por evento; favorito tem a menor odd
...
def test_select_fav_dog_draw():
    ...
    # evento 0: fav=H (menor odd), dog=A (maior odd)
    assert fav.o.iloc[0] == 1.5 and dog.o.iloc[0] == 7.0
    # evento 1: fav=A, dog=H
...
def test_two_outcome_market_no_draw():
    ...
    # sem papel de empate → seleção vazia → assinatura None
...
def test_validate_catches_bad_frame():
    ...
    bad = df.copy()
    bad.loc[bad.outcome == "D", "won"] = 1          # dois vencedores
