...
# PocketBase is only the backend in the PocketBase tier. In the Hauldr tier
# (AUTH_MODE=hauldr / DATA_MODE=postgrest) the app talks to GoTrue + PostgREST +
# Garage, so starting PB would just waste memory — skip it entirely.
if [ "${AUTH_MODE:-pocketbase}" = "pocketbase" ] || [ "${DATA_MODE:-pocketbase}" = "pocketbase" ]; then
  ...
  echo "[entrypoint] Hauldr tier (AUTH_MODE=${AUTH_MODE} / DATA_MODE=${DATA_MODE}) — skipping PocketBase"
fi
